...
void test_packet_queue()
{
	...
	theNet::PacketQueue packetQueue;
	
	printf( "check insert back\n" );
	for ( int i = 0; i < 100; ++i )
	{
		theNet::PacketData data;
		data.sequence = i;
...
void test_reliability_system()
{
	...
	check( theNet::ReliabilitySystem::bit_index_for_sequence( 99, 100, MaximumSequence ) == 0 );
	check(theNet::ReliabilitySystem::bit_index_for_sequence( 90, 100, MaximumSequence ) == 9 );
	check(theNet::ReliabilitySystem::bit_index_for_sequence( 0, 1, MaximumSequence ) == 0 );
	check(theNet::ReliabilitySystem::bit_index_for_sequence( 255, 0, MaximumSequence ) == 0 );
	check(theNet::ReliabilitySystem::bit_index_for_sequence( 255, 1, MaximumSequence ) == 1 );
	check(theNet::ReliabilitySystem::bit_index_for_sequence( 254, 1, MaximumSequence ) == 2 );
	check(theNet::ReliabilitySystem::bit_index_for_sequence( 254, 2, MaximumSequence ) == 3 );
	...
	check(theNet::ReliabilitySystem::generate_ack_bits( 32, packetQueue, MaximumSequence ) == 0xFFFFFFFF );
	check(theNet::ReliabilitySystem::generate_ack_bits( 31, packetQueue, MaximumSequence ) == 0x7FFFFFFF );
	check(theNet::ReliabilitySystem::generate_ack_bits( 33, packetQueue, MaximumSequence ) == 0xFFFFFFFE );
	check(theNet::ReliabilitySystem::generate_ack_bits( 16, packetQueue, MaximumSequence ) == 0x0000FFFF );
	check(theNet::ReliabilitySystem::generate_ack_bits( 48, packetQueue, MaximumSequence ) == 0xFFFF0000 );
	...
	check(theNet::ReliabilitySystem::generate_ack_bits( 0, packetQueue, MaximumSequence ) == 0xFFFFFFFF );
	check(theNet::ReliabilitySystem::generate_ack_bits( 255, packetQueue, MaximumSequence ) == 0x7FFFFFFF );
	check(theNet::ReliabilitySystem::generate_ack_bits( 1, packetQueue, MaximumSequence ) == 0xFFFFFFFE );
	check(theNet::ReliabilitySystem::generate_ack_bits( 240, packetQueue, MaximumSequence ) == 0x0000FFFF );
	check(theNet::ReliabilitySystem::generate_ack_bits( 16, packetQueue, MaximumSequence ) == 0xFFFF0000 );
	
	printf( "check process ack (1)\n" );
	{
		theNet::PacketQueue pendingAckQueue;
		for ( int i = 0; i < 33; ++i )
		{
			theNet::PacketData data;
			data.sequence = i;
		...
		theNet::PacketQueue ackedQueue;
		std::vector<unsigned int> acks;
		float rtt = 0.0f;
		unsigned int acked_packets = 0;
		theNet::ReliabilitySystem::process_ack( 32, 0xFFFFFFFF, pendingAckQueue, ackedQueue, acks, acked_packets, rtt, MaximumSequence );
		check( acks.size() == 33 );
		...
		for ( unsigned int i = 0; i < acks.size(); ++i )
			check( acks[i] == i );
		unsigned int i = 0;
		for (theNet::PacketQueue::iterator itor = ackedQueue.begin(); itor != ackedQueue.end(); ++itor, ++i )
	...
	printf( "check process ack (2)\n" );
	{
		...
		float rtt = 0.0f;
		unsigned int acked_packets = 0;
		theNet::ReliabilitySystem::process_ack( 32, 0x0000FFFF, pendingAckQueue, ackedQueue, acks, acked_packets, rtt, MaximumSequence );
		check( acks.size() == 17 );
		...
		ackedQueue.verify_sorted( MaximumSequence );
		unsigned int i = 0;
		for (theNet::PacketQueue::iterator itor = pendingAckQueue.begin(); itor != pendingAckQueue.end(); ++itor, ++i )
			check( itor->sequence == i );
		i = 0;
		for (theNet::PacketQueue::iterator itor = ackedQueue.begin(); itor != ackedQueue.end(); ++itor, ++i )
	...
	printf( "check process ack (3)\n" );
	{
		...
		float rtt = 0.0f;
		unsigned int acked_packets = 0;
		theNet::ReliabilitySystem::process_ack( 48, 0xFFFF0000, pendingAckQueue, ackedQueue, acks, acked_packets, rtt, MaximumSequence );
		check( acks.size() == 16 );
	...
	printf( "check process ack wrap around (1)\n" );
	{
		...
		float rtt = 0.0f;
		unsigned int acked_packets = 0;
		theNet::ReliabilitySystem::process_ack( 0, 0xFFFFFFFF, pendingAckQueue, ackedQueue, acks, acked_packets, rtt, MaximumSequence );
		check( acks.size() == 33 );
		check( acked_packets == 33 );
		check( ackedQueue.size() == 33 );
		check( pendingAckQueue.size() == 0 );
	...
	printf( "check process ack wrap around (2)\n" );
	{
		...
		float rtt = 0.0f;
		unsigned int acked_packets = 0;
		theNet::ReliabilitySystem::process_ack( 0, 0x0000FFFF, pendingAckQueue, ackedQueue, acks, acked_packets, rtt, MaximumSequence );
		check( acks.size() == 17 );
		check( acked_packets == 17 );
		check( ackedQueue.size() == 17 );
		check( pendingAckQueue.size() == 33 - 17 );
	...
	printf( "check process ack wrap around (3)\n" );
	{
		...
		float rtt = 0.0f;
		unsigned int acked_packets = 0;
		theNet::ReliabilitySystem::process_ack( 16, 0xFFFF0000, pendingAckQueue, ackedQueue, acks, acked_packets, rtt, MaximumSequence );
		check( acks.size() == 16 );
		check( acked_packets == 16 );
		check( ackedQueue.size() == 16 );
		check( pendingAckQueue.size() == 16 );
...
void test_join()
{
	...
	theNet::ReliableConnection client( ProtocolId, TimeOut );
	theNet::ReliableConnection server( ProtocolId, TimeOut );
	
	check( client.Start( ClientPort ) );
	check( server.Start( ServerPort ) );
	
	client.Connect(theNet::Address(a,b,c,d,ServerPort ) );
	server.Listen();
...
void test_join_busy()
{
	...
	theNet::ReliableConnection busy( ProtocolId, TimeOut );
	check( busy.Start( ClientPort + 1 ) );
	busy.Connect(theNet::Address(a,b,c,d,ServerPort ) );
...
void test_sequence_wrap_around()
{
	...
	theNet::ReliableConnection client( ProtocolId, TimeOut, MaxSequence );
	theNet::ReliableConnection server( ProtocolId, TimeOut, MaxSequence );
